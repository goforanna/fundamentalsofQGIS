# Visualizing Data
Congratulations, you've made it to the fun part of mapping! Playing with data visualization in GIS is where personality, creativity, and design really get a chance to shine. It is how data transforms into a compelling narrative. We will do this by adjusting the **symbology** of the Count layer. Symbology is the use of graphic elements (color, size, shape, transparency) to represent geographic features on a map. While a raw dataset might contain coordinates and numbers, symbology is what turns that data into a visual story. It determines how a point, line, or polygon appears to on the map based on its attributes.

Currently, the Count layer is symbolized as a single color. In this section you will change the symbology so that the polygons--the NYC census block groups--vary in color according to the number of complaints filed within each group.  

Make sure that the Counts layer is at the top of the drawing order.

## Applying Graduated Symbology to a Polygon Layer

In order to adjust the symbology of the Count layer, right-click the layer in the Layers Panel and select Properties. This opens the Layer Properties window. Select the "Symbology" tab from the left hand side. 
...
First, we need to determine the attribute field that the polygons will be graduated by. Click the empty drop-down box to the right of "Value," and you'll see just the fieldnames for just the numeric attribute data in the Counts layer. 
...
Adjust the Symbology window so that both the window and the map canvas are visible. Then, select Natural Breaka and click **classify**. 6 classes populate in the middle of the Symbology window. Notice how the layer has been re-rendered to symbolize each block group by one of the six colors 
